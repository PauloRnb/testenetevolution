...
const tabTriggerClass = `
  w-[160px] px-5 py-2 text-base font-medium
  text-zinc-700 hover:text-blue-500 focus:text-blue-500 focus:outline-none
  data-[selected]:text-blue-700
  dark:text-white dark:hover:text-cyan-500 dark:focus:text-cyan-500 dark:data-[selected]:text-cyan-400
`;

export default function SectionOfferHome() {
  return (
        ...
        <div className="container pt-8">
          <Tabs.Root defaultValue="combos">
            <Tabs.List className="relative border-b-[8px] border-zinc-200 dark:border-zinc-700">
              <Tabs.Trigger value="combos" className={tabTriggerClass}>
                Combos
              </Tabs.Trigger>
              <Tabs.Trigger value="internet" className={tabTriggerClass}>
                Internet
              </Tabs.Trigger>

              <Tabs.Indicator
                className="absolute bottom-[-6px] ml-1 h-1 rounded-full bg-blue-700 transition-all duration-300 dark:bg-cyan-400"
                style={{ width: 160 }}
              />
            </Tabs.List>
