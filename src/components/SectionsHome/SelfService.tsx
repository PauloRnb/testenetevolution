...
const cardItems = [
  ...
  },
  {
    ...
    description:
      "Veja suas faturas, seu plano e seu consumo através da central do assianate.",
    href: "https://netevolution.sgp.tsmx.com.br/accounts/central/login/",
  },
  {
    icon: LaptopMinimal,
...
function Card({ icon: Icon, title, description, href }: (typeof cardItems)[0]) {
  return (
    <a
      ...
      className="group flex min-h-[14.063rem] w-full flex-col justify-between rounded-xl bg-zinc-100 p-4 shadow-sm hover:bg-blue-700 dark:bg-zinc-900 dark:hover:bg-zinc-900 dark:hover:ring-2 dark:hover:ring-cyan-400"
    >
      ...
      <div className="flex flex-col gap-y-4">
        <div className="flex flex-col gap-1">
          <h3 className="text-base font-semibold leading-4 text-zinc-800 group-hover:text-white dark:text-white">
            {title}
          </h3>
          <Separator className="!h-[2px] !w-10 rounded-full bg-blue-700 group-hover:bg-white dark:bg-white" />
        </div>
        <p className="text-sm font-medium text-zinc-500 group-hover:text-zinc-100 dark:text-white">
          {description}
        </p>
      </div>
